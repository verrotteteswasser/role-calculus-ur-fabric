...
\begin{titlepage}
  ...
  \vfill
  \includegraphics[width=0.28\linewidth]{figures/logo-placeholder}
\end{titlepage}
...
\chapter{Benchmark I: Newtonian Gravity ⇒ Orientation Law (T1–Grav)}\label{bench:grav}
\section*{Prediction}
Stacks around approximately spherical deficits/excesses of radius $a$ exhibit inner slope $+1$, outer slope $-2$, and the parameter-free identity $\Cout/\sinIn=a^3$ with $v_O\!\leftrightarrow g$.

\section*{Protocol}
Stack objects by $a$; estimate the interior slope $s_{\rm in}$ from $g_r(r\!\le\!a)$ and the exterior Gauss constant $C_{\rm out}:=r^2 g_r|_{r\ge a}$; verify slopes and identity; obtain $G$ from interior/exterior pulls and report a weighted $G^*$.

\section*{Nulls}
Label shuffles (reassign $a$), shell randomization of profiles, independent pipeline splits.

\section*{Report}
Slopes with CIs, identity residual $\delta_I=\big|\Cout/\sinIn-a^3\big|/a^3$, $G_{\rm in},G_{\rm out},G^*$; bootstrap/jackknife bands.

\addfig{figures/gravity.png}{Gravity as orientation: $+1/-2$ slopes and the identity $\Cout/\sinIn=a^3$.}{fig:bench:gravity}[0.9]

\chapter{Benchmark II: Electrostatics ⇒ Orientation Law (T1–EM)}\label{bench:em}
\section*{Prediction}
Replacing masses by charges and $G$ by $1/(4\pi\varepsilon_0)$ yields the same signatures: inner $+1$, outer $-2$, $\Cout/\sinIn=a^3$.

\section*{Protocol}
Use dielectric/charge reconstructions to build stacks; repeat the T1 steps with $\kO^{(\mathrm{EM})}=1/(4\pi\varepsilon_0)$.

\section*{Nulls}
As in Benchmark I; add electrode/geometry randomization where applicable.

\section*{Report}
Slope fits, identity residual, cross-check of $\kO^{(\mathrm{EM})}$ across environments.

\addfig{figures/electrostatics.png}{Electrostatics as orientation: same inner/outer slopes and identity with $\kO^{(\mathrm{EM})}=1/(4\pi\varepsilon_0)$.}{fig:bench:electrostatics}[0.9]

\chapter{Benchmark III: Baryon-Conditioned Radiation (Carrier–Imprint)}\label{bench:baryon}
\section*{Prediction}
Transport remains $O$-driven (carrier asymptotes), while binding shells $b(r)$ imprint emission/attenuation (shape). Environment-dependent $q(r)$ and $J(r)$ share the same carrier asymptotes.

\section*{Protocol}
Fit profiles with a carrier–imprint form (carrier asymptote fixed; imprint via $b(r)$ family); compare environments (e.g., low/high $b$).

\section*{Nulls}
Swap $b$-shell labels; randomized shell radii; pipeline splits.

\section*{Report}
Asymptotic exponents (carrier) vs.\ imprint parameters; goodness-of-fit and robustness under nulls.

\addfig{figures/baryon_conditioning.png}{Baryon conditioning: payload and flux shaped by $b(r)$ on top of an $O$-driven carrier.}{fig:res:baryon-conditioning}[0.9]

\chapter{Benchmark IV: Sensitivity to Source/Sink Parameters}\label{bench:sensitivity}
\section*{Prediction}
One-at-a-time variation separates \emph{shape} controls from \emph{amplitude} controls and identifies robust role-level features.

\section*{Protocol}
Scan $(s_0,\lambda_0,\mu,\nu,\nu')$ over plausible ranges; classify parameters by their effect on near- vs.\ far-field behavior.

\section*{Nulls}
Randomize parameter–profile pairings; compare to surrogate scans with matched marginals.

\section*{Report}
Spider or waterfall summaries; near/far sensitivity split; parameters with shape control vs.\ amplitude only.

\addfig{figures/sensitivity.png}{Sensitivity families across $(s_0,\lambda_0,\mu,\nu,\nu')$; near- vs.\ far-field control distinguished.}{fig:res:sensitivity}[0.9]

\chapter{Benchmark V: Stacking of Multiple Structures}\label{bench:stacking}
\section*{Prediction}
Ensemble averaging smooths local features yet preserves net conditioning and carrier identities—explaining smooth sky maps that still respect role signatures.

\section*{Protocol}
Construct representative vs.\ ensemble stacks; compare asymptotes and identity residuals.

\section*{Nulls}
Shuffle membership labels; rebin radii; independent selection masks.

\section*{Report}
Ensemble mean $\pm1\sigma$ vs.\ representative profiles; stability of T1 signatures under stacking.

\addfig{figures/stacking.png}{Stacking: representative vs.\ ensemble mean with $\pm1\sigma$ band; net conditioning preserved.}{fig:res:stacking}[0.9]

\chapter{Benchmark VI: Thresholds and Hysteresis}\label{bench:hysteresis}
\section*{Prediction}
Control sweeps reveal nucleation and persistence with on/off asymmetry and a nonzero loop area—the operational face of $\Theta$ and $\sigma_c$.
...
% References in TOC with a clear title
