...
\usepackage{setspace}
\onehalfspacing
% kein zweites \usepackage{microtype} hier
...
\usepackage{csquotes} % empfohlen mit biblatex
\usepackage[
  backend=biber,
  style=authoryear,     % oder numeric / alphabetic / apa
  sorting=nyt,
  natbib=true           % <— ERLAUBT \citep, \citet etc.
]{biblatex}
\ExecuteBibliographyOptions{uniquename=init} % behebt die inits/uniquename-Warnung
...
% Roles
\node[rectangle,draw,rounded corners,below left=1.8cm of N,minimum width=2.2cm] (O) {Orientation \\ ($O=C\circ N$)};
\node[rectangle,draw,rounded corners,below right=1.8cm of S,minimum width=2.2cm] (G) {Binding \\ ($G=C\circ S$)};
